...
export async function POST(_request: NextRequest) {
  try {
    const res = await fetch(`${BACKEND_BASE}/system/kill-switch`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: 'Backend /system/kill-switch failed', detail: text },
        { status: res.status }
      )
    }
    ...
    return NextResponse.json(
      { error: 'Failed to reach backend /system/kill-switch' },
      { status: 502 }
    )
  }
}
