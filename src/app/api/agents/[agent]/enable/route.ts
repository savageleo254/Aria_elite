...
export async function POST(request: NextRequest, { params }: { params: { agent: string } }) {
  try {
    ...
    const res = await fetch(`${BACKEND_BASE}/agents/${agentName}/enable`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: `Backend /agents/${agentName}/enable failed`, detail: text }, { status: res.status })
    }

    const data = await res.json()
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json({ error: 'Failed to reach backend agent enable endpoint' }, { status: 502 })
  }
}
